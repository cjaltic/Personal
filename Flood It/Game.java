...
public class Game {
  ...
  private List<Integer> thisRun = new DoublyLinkedList<>();  // for timings
  ...
  private void init() {
    ...
    currentStep = 0;
    if (interactive) {
      // perform the initial flood
      select(board.get(Coord.ORIGIN).getColor());
  ...
  public void select(WaterColor color) {
    select(0, color); // k == 0 means use Board.flood() as the flood function
  }
  ...
  public void select(int k, WaterColor color) {
    try {
      ...
      System.out.println("Something went terribly wrong with the " +
          "flood reflection!");
      e.printStackTrace();
    }
  }
  ...
  private void autoPlay(int k) {
    long gameTime = 0;
    // perform the initial flood
    select(k, board.get(Coord.ORIGIN).getColor());
  ...
  private void batchTest() {
    List<List<Integer>> allTimings = new DoublyLinkedList<>();
    Class<?> c = Board.class; 
    Class<?>[] argTypes = new Class[] { WaterColor.class };
    int k = 0;
    try {
      while (true) {
        String name = "flood" + (k == 0 ? "" : k);
        c.getDeclaredMethod(name, argTypes); // throws if no such method
        System.out.println("running with " + name + " as the flood function");
        thisRun = new DoublyLinkedList<>();
        for (int size = 1;
             size <= Constants.MAX_BOARD_SIZE_FOR_AUTOPLAY;
             size++) {
