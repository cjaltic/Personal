...
public class TimingGraph extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    // Draw x-axis
    g2.draw(new Line2D.Double(numRuns, h - numRuns,
        w - numRuns, h - numRuns));
    // Draw labels
    ...
    String s = "time";
    float sy = numRuns + ((h - 2 * numRuns) - s.length() * sh) / 2 +
        lm.getAscent();
  ...
  private void showAndTell() {
    ...
    try {
      ...
      System.out.println("Unable to write graph to the file " +
          Constants.TEST_GRAPH);
    }
  }
}
